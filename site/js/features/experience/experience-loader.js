...
(function(g){
 ...
 function ensure(){if(!pending)pending=(g.RadarExperienceCore?Promise.resolve():script('experience-core.js')).then(()=>g.RadarExperience?null:script('experience.js')).then(()=>{g.RadarExperience.configure(config,api);return g.RadarExperience;}).catch(e=>{pending=null;throw e;});return pending;}
 let newsPending;
 function newsStyle(){if(document.getElementById('news-styles'))return Promise.resolve();return new Promise((resolve,reject)=>{const l=document.createElement('link');l.id='news-styles';l.rel='stylesheet';l.href='news.css';l.onload=resolve;l.onerror=()=>{l.remove();reject(new Error('News stylesheet failed'));};document.head.append(l);});}
 function ensureNews(){if(!newsPending)newsPending=Promise.all([newsStyle(),(g.RadarNewsCore?Promise.resolve():script('news-core.js')).then(()=>g.RadarNews?null:script('news.js'))]).then(()=>{g.RadarNews.configure(config,api);return g.RadarNews;}).catch(e=>{newsPending=null;throw e;});return newsPending;}
 let generation=0;

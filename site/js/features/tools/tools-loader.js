...
(function(g){
 ...
 function style(){if(document.getElementById('daily-tools-css'))return Promise.resolve();return new Promise((resolve,reject)=>{const s=document.createElement('link');s.id='daily-tools-css';s.rel='stylesheet';s.href='styles/features/tools.css?v=final-20260918';s.onload=resolve;s.onerror=()=>{s.remove();reject(Error('样式加载失败'));};document.head.append(s);});}
 function ensure(){if(!pending)pending=Promise.all([style(),(g.RadarToolsCore?Promise.resolve():js('js/features/tools/tools-core.js?v=final-20260918')).then(()=>g.RadarDaily?null:js('js/features/tools/tools.js?v=final-20260918'))]).then(()=>{g.RadarDaily.configure(config,api);return g.RadarDaily;}).catch(e=>{pending=null;throw e;});return pending;}

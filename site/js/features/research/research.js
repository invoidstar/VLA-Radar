...
(function (global) {
  ...
  function richEvidence(note){
    ...
    const review=br?`<aside class="note-benchmark-review"><strong>评测提取：${esc(labels[br.status])}</strong><div class="note-small">${para(br.note)}</div><small>检查时间：${esc(br.checkedAt||'尚未完成')}</small></aside>`:'';

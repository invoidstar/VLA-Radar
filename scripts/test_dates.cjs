...
for (const input of [null, undefined, '', '2026-08', '2026', '2026-02-29', '2026-02-30', '2026-13-01', '2026-01-00', '2026-8-1', '2026-08-01T00:00:00Z', 20260801]) {
  expect(Dates.isoWeek(input), null);
}
expect(Dates.fromKey('2025-W53'), null);
expect(Dates.fromKey('2026-W54'), null);
expect(Dates.fromKey('2026-W00'), null);
expect(Dates.fromKey('2026-W1'), null);
expect(Dates.fromKey('2020-W53').start, '2020-12-28');
expect(Dates.weeksInYear('2026').length, 53);
expect(Dates.weeksInYear('2025').length, 52);
expect(Dates.weeksInYear('not-a-year'), []);
expect(Dates.weeksInYear('1000').every(Boolean), true);
expect(Dates.weeksInYear('9999').every(Boolean), true);
expect(Dates.label('2026-W35'), '2026-W35 · 2026-08-24—2026-08-30');
expect(Dates.label('undated'), '日期未精确到日');
// Cross-check every calendar day in 1990–2040 against Python's independent ISO calendar.
...
for(const [date,key,start,end] of rows){
  const actual=Dates.isoWeek(date);
  expect([actual.key,actual.start,actual.end],[key,start,end]);
  expect(Dates.fromKey(key),actual);
}

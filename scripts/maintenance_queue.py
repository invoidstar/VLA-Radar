...
def plan(root,limit=8):
    ...
    write(Path(root)/'maintenance/work-queue.json',out);return out
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('--root',type=Path,default=Path(__file__).resolve().parents[1]);ap.add_argument('--limit',type=int,default=8);a=ap.parse_args();print(plan(a.root,a.limit))

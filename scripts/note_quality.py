...
def validate_note_extras(note, public_url, require):
    ...
    if cov is not None:
        ...
        require(cov['level'] == 'deep' and cov['scope'] in SCOPES, 'note coverage level/scope')
        public_url(cov['source'])
        require(len(note['sections']) >= 8, 'deep notes need eight substantive sections')
        require(sum(len(s['body']) for s in note['sections']) >= 1200, 'deep note is too short')
        require(all(len(s['body']) >= 90 and s['sources'] for s in note['sections']), 'deep sections need explanatory content and sources')

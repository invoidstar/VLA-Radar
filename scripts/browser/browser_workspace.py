...
server=subprocess.Popen(['python','-m','http.server',str(port),'--bind','127.0.0.1'],cwd=PUBLIC_ROOT,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL);time.sleep(.3)
...
try:
 with sync_playwright() as p:
  ...
  page.goto(base+'?view=leaderboards&dataset=CALVIN',wait_until='networkidle');page.wait_for_selector('.setting-summary')
  yes('workspace reaches Setting-first Benchmark view',page.locator('.setting-table').count()==1)
  page.locator('.setting-advanced-link').click();page.wait_for_selector('#show-protocol-chart');page.locator('#show-protocol-chart').click();page.wait_for_selector('.chart-row')

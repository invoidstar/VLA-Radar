...
server = subprocess.Popen([sys.executable, '-m', 'http.server', str(port), '--bind', '127.0.0.1'],
                          cwd=PUBLIC_ROOT, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
try:
    time.sleep(.3)
    with sync_playwright() as p:
        ...
        yes('four primary navigation areas', page.locator('[data-nav-group]').evaluate_all('(els)=>els.map(e=>e.dataset.navGroup)') == ['discover','evidence','workspace','personal'])
        yes('discover owns topic shortcuts', page.locator('[data-nav-group="discover"] #side-topics').count() == 1)
        yes('evidence owns benchmark and coverage', page.locator('[data-nav-group="evidence"] [data-view="leaderboards"]').count() == 1 and page.locator('[data-nav-group="evidence"] [data-view="coverage"]').count() == 1)
        yes('workspace owns compare updates export', page.locator('[data-nav-group="workspace"] [data-view="compare"]').count() == 1 and page.locator('[data-nav-group="workspace"] [data-view="updates"]').count() == 1 and page.locator('[data-nav-group="workspace"] [data-nav-action="export"]').count() == 1)
        yes('personal owns local views', page.locator('[data-nav-group="personal"] [data-view="radar"]').count() == 1 and page.locator('[data-nav-group="personal"] [data-view="reading"]').count() == 1)
        yes('about and GitHub live in footer', page.locator('.side-bottom [data-view="about"]').count() == 1 and page.locator('.side-bottom a.side-external').count() == 1)
        ...
        topic_panel.locator('summary').click()
        page.locator('[data-nav-action="export"]').click(); page.wait_for_selector('#workspace-export[open]')
        yes('workspace export opens from sidebar without navigation', page.locator('#workspace-export').get_attribute('aria-labelledby') == 'export-title')
        page.locator('#export-close').click(); yes('workspace export closes normally', page.locator('#workspace-export[open]').count() == 0)

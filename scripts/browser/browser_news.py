...
subprocess.run(['python',str(ROOT/'scripts/build/stage_site.py'),'--output',str(PUBLIC_ROOT)],check=True);OUT=Path(os.environ.get('RADAR_NEWS_OUTPUT','/tmp/radar-news-browser'));OUT.mkdir(parents=True,exist_ok=True)
s=socket.socket();s.bind(('127.0.0.1',0));port=s.getsockname()[1];s.close()
server=subprocess.Popen(['python','-m','http.server',str(port),'--bind','127.0.0.1'],cwd=PUBLIC_ROOT,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL);time.sleep(.4)

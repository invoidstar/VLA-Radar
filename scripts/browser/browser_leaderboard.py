...
try:
    with sync_playwright() as p:
        ...
        # Default Benchmark path is Setting-first for every dataset; track= remains advanced compatibility.
        all_settings=boards.get('settings',[])
        rc=next(s for s in all_settings if s['dataset']=='RoboCasa365' and {'robocasa365-official-100','robocasa365-official-101'}<=set(s['trackIds']) and s['paperCount']>=3)
        ...
        rm=next(s for s in all_settings if set(s['trackIds'])=={'robomimic-corl21-lowdim-ph-best','robomimic-corl21-image-ph-best'})
        ...
        li=next(s for s in all_settings if set(s['trackIds'])=={'libero-infoentropy-v1-budget60','libero-infoentropy-v1-budget80','libero-infoentropy-v1-budget120','libero-infoentropy-v1-budget140'})
        ...
        yes('recipe-only budget variants share one Setting',page.locator('.setting-method strong').all_text_contents().count('VLA-InfoEntropy')==4)

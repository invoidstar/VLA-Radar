...
ROOT=Path(__file__).resolve().parents[1];OUT=Path(os.getenv('RADAR_TOOLS_OUTPUT','/tmp/radar-tools'));OUT.mkdir(parents=True,exist_ok=True)
...
server=subprocess.Popen([sys.executable,'-m','http.server',str(port),'--bind','127.0.0.1'],cwd=ROOT,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL);checks=[];errors=[];requests=[]
def yes(label,value):
 assert value,label
 checks.append(label)
try:

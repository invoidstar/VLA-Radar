...
def sync(root,limit=100,apply=False,pause=3.1,due_days=7):
    ...
    if apply:
        # No changes to maintenance/state/state.json: a metadata scan is not a literature-discovery sweep.
        write(root/'maintenance/state/publication-check.json',report)
        prev=load(root/'maintenance/state/publication-candidates.json') if (root/'maintenance/state/publication-candidates.json').exists() else {'candidates':[]}
        keyed={(x['paperId'],x['kind'],x['value']):x for x in prev['candidates']}
        for c in candidates:keyed[(c['paperId'],c['kind'],c['value'])]=c
        write(root/'maintenance/state/publication-candidates.json',{'schemaVersion':1,'updatedAt':when,'candidates':list(keyed.values())})
        if changed:m['updatedAt']=when;write(root/'catalog/manifest.json',m)
    return report
if __name__=='__main__':
    ap=argparse.ArgumentParser(description=__doc__);ap.add_argument('--root',type=Path,default=Path(__file__).resolve().parents[2]);ap.add_argument('--limit',type=int,default=100);ap.add_argument('--due-days',type=int,default=7);ap.add_argument('--apply-safe',action='store_true');a=ap.parse_args()

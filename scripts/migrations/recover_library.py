...
def run(root,retained=None):
    marker=root/'maintenance/audits/migrations/library-recovery.json'
    ...
    for name in ['scripts/discovery/note_quality.py','scripts/build/catalog_core.py','research.js','research.css','tests/node/test_research.cjs']:
        (root/name).write_text(get(name),encoding='utf-8')
    p=root/'scripts/discovery/note_quality.py';s=p.read_text();s=s.replace("require(cov['level'] == 'deep' and cov['scope'] in SCOPES, 'note coverage level/scope')", "require(cov['level'] in {'deep','limited'} and cov['scope'] in SCOPES, 'note coverage level/scope')\n        require((cov['level']=='limited') == (cov['scope']=='official-abstract-only'), 'abstract-only cannot be promoted to full-source notes')")
    ...
    p=root/'scripts/build/catalog_core.py';s=p.read_text().replace('from pathlib import Path','from pathlib import Path\nfrom zoneinfo import ZoneInfo')
...
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('--root',type=Path,default=Path(__file__).resolve().parents[2]);ap.add_argument('--retained',type=Path);a=ap.parse_args();run(a.root,a.retained)

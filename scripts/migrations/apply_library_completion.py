...
def install_scope_support(root):
    p=root/'scripts/discovery/note_quality.py';s=p.read_text(encoding='utf-8')
    ...
    p.write_text(s,encoding='utf-8')
    p=root/'scripts/validate/validate_all.py';s=p.read_text(encoding='utf-8')
    if "'scripts/validate/check_editorial.py'" not in s:
        s=s.replace("commands=[[sys.executable,'scripts/validate/validate_schema.py'],", "commands=[[sys.executable,'scripts/validate/validate_schema.py'],[sys.executable,'scripts/validate/check_editorial.py'],")
    p.write_text(s,encoding='utf-8')


def apply(root=ROOT):
    marker=root/'maintenance/audits/migrations/library-completion.json'
    if marker.exists():
    ...
    result_source=root/'maintenance/audits/deep-reading/retained-results.json'
    ...
    write(root/'maintenance/state/benchmark-review.json',ledger)
    ...
    write(root/'maintenance/policies/editorial-policy.json',{'schemaVersion':1,'effectiveAt':STAMP,'baselinePaperIds':sorted(records),'limitedLegacyIds':LIMITED,'newPaperMinimumCharacters':2000,'minimumSections':8,'weeklyMergeMode':'self-review-ci-then-merge','scope':'Counts are structural checks, not independent scientific certification.'})

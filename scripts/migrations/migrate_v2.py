...
def main():
    ...
    write(ROOT/'maintenance/audits/migrations/v2-migration.json',{'schemaVersion':1,'migratedAt':WHEN,'papers':len(order),'expandedNotes':list(NOTES),'results':len(results),'tracks':len(tracks),'fullLibraryReverified':False,'firstPublicDatesChanged':0,'keyResultsChanged':0,'notes':'旧笔记未冒充重新读全文；新版本元数据与笔记核验分开。'})

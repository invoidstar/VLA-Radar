...
try:
 with sync_playwright() as p:
  ...
  yes('optional script loaded only on route',any('/news.js' in u for u in requests));yes('partial scan visibly labelled','部分检索' in page.locator('#news-issue-status').inner_text());yes('unknown weeks not shown as zero',all('未知' in a for a in page.locator('.news-week-bar.unknown').evaluate_all('(els)=>els.map(e=>e.getAttribute("aria-label"))')))

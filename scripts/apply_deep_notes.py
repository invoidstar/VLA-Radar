"""Import original, source-linked reading batches; preserve legacy paper facts."""
from __future__ import annotations
import argparse, importlib, re
from pathlib import Path
import catalog_core
from catalog_core import load, write, today
...
def apply(root: Path, inputs: Path):
    ...
    for path in sorted(inputs.glob('batch-*.json')):
        ...
        for x in batch:
            ...
            oldreview=rec['note'].get('benchmarkReview');count=sum(r['paperId']==pid and r['evidence']=='checked' for r in results)
    ...
    m,records,tracks,results=catalog_core.read_catalog(root)
    if changed:m['updatedAt']=when;write(root/'catalog/manifest.json',m)
    deep=[r for r in records if r['note'].get('coverage',{}).get('level')=='deep'];limited=[r['paper']['id'] for r in deep if r['note']['coverage']['scope']=='official-abstract-only']
    report={'schemaVersion':1,'checkedAt':when,'totalPapers':len(records),'deepNotes':len(deep),'fullMethodOrTheoryOrReportNotes':len(deep)-len(limited),'abstractOnlyNotes':limited,
            ...
            'statement':'Imported original summaries with per-section sources. Structure checks are not scientific verification. Abstract-only entries are not full-paper certification; lifecycle and discovery checkpoints are independent.'}
    write(root/'maintenance/deep-notes-progress.json',report);print('Applied changes:',changed);print(report);return report

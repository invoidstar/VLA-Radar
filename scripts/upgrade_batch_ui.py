...
p=R/'scripts/catalog_core.py';s=p.read_text();old="require(t['dataset'] in {'LIBERO','RoboTwin','RoboCasa'},'supported dataset family')";new="require(isinstance(t['dataset'],str) and re.fullmatch(r'[A-Za-z][A-Za-z0-9 +._-]{1,50}',t['dataset']),'invalid dataset family')"
...
p=R/'scripts/validate_all.py';s=p.read_text()
if 'test_content.cjs' not in s:p.write_text(s.replace("['node','scripts/test_research.cjs']]","['node','scripts/test_research.cjs'],['node','scripts/test_content.cjs']]"),encoding='utf-8')

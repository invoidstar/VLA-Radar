...
(function (global) {
  ...
  function datasetNames(data){return [...new Set(data.tracks.map(t=>t.dataset))];}
  ...
  async function renderBoards(host){
    ...
    try{
      ...
      const query=new URLSearchParams(location.search);const datasets=datasetNames(data);let dataset=datasets.includes(query.get('dataset'))?query.get('dataset'):(datasets[0]||'');let trackId=query.get('track')||'',metric='',page=1;
      function draw(){
  ...
  global.RadarResearch={enhance,renderBoards,pageWindow,rankRows,visibleResults,datasetNames,formatScore,noteBlocks};

...
(function (global) {
  ...
  async function enhance(p,root){
    ...
    try{
      ...
      panel.innerHTML=`<div class="note-heading"><span class="note-badge ${note.status==='expanded'?'verified':''}">${note.status==='expanded'?'已扩展 · 有原文定位':stale?'笔记已扩展 · 新版本待复核':'既有笔记 · 待深化核验'}</span><small>笔记核验 ${esc(date(note.verifiedAt))}</small></div><div class="research-tabs" role="tablist" aria-label="论文详情内容"><button id="tab-notes" role="tab" aria-selected="true" aria-controls="panel-notes" data-note-tab="notes">阅读笔记</button><button id="tab-life" role="tab" aria-selected="false" aria-controls="panel-life" tabindex="-1" data-note-tab="life">发表历程</button><button id="tab-results" role="tab" aria-selected="false" aria-controls="panel-results" tabindex="-1" data-note-tab="results">评测记录</button></div><div id="panel-notes" role="tabpanel" aria-labelledby="tab-notes"><div class="note-context"><strong>阅读版本：${esc(note.version)}</strong><p>${note.status==='legacy'?'下面保留原有公开笔记，尚未逐篇重新读全文。周更会按优先级和核验时间补齐研究问题、方法机制、创新差异、实验和消融；不会用重复模板冒充完整精读。':stale?'下方结论有具体来源，但阅读版本早于当前 arXiv 版本。旧版结果保留，版本差异待核验。':'以下是公开原文指定片段的结构化总结，并非独立复现或整篇认证。'}</p></div><section class="key-result-full"><small>KEY RESULT · 原记录保留</small>${para(p.findings)}</section><div class="note-toc" aria-label="笔记章节">${note.sections.map((s,i)=>`<button data-note-anchor="note-${esc(s.id)}">${String(i+1).padStart(2,'0')} ${esc(s.title)}</button>`).join('')}</div>${note.sections.map((s,i)=>`<section class="note-section" id="note-${esc(s.id)}"><span class="note-number">${String(i+1).padStart(2,'0')}</span><div><h3>${esc(s.title)}</h3>${para(s.body)}${links(s.sources)}</div></section>`).join('')}</div><div id="panel-life" role="tabpanel" aria-labelledby="tab-life" hidden>${lifecycle(r.publication,p)}</div><div id="panel-results" role="tabpanel" aria-labelledby="tab-results" hidden><p class="research-loading">正在载入公开评测记录…</p></div>`;
  ...
  async function renderBoards(host){
    ...
    try{
      ...
      const query=new URLSearchParams(location.search);let dataset=['LIBERO','RoboTwin','RoboCasa'].includes(query.get('dataset'))?query.get('dataset'):'LIBERO';let trackId=query.get('track')||'',metric='',page=1;
      function draw(){
  ...
  global.RadarResearch={enhance,renderBoards,pageWindow,rankRows,visibleResults};

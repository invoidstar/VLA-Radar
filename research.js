...
(function (global) {
  ...
  async function renderBoards(host){
    ...
    try{
      ...
      async function sync(){await draw();const u=new URL(location.href);u.searchParams.set('dataset',dataset);u.searchParams.set('track',trackId);history.replaceState({},'',u);}
      await draw();
  ...
  global.RadarResearch={configure,detail,enhance,renderBoards,pageWindow,rankRows,visibleResults,metricValue,benchmarkFamilies,richEvidence,datasetNames,formatScore,noteBlocks};

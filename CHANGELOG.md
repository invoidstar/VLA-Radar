...
- 接入不蒜子 v3.6.9，仅在正式 `invoidstar.github.io` Pages 域名加载统计，避免本地开发 / CI 访问污染全站访问量与访客数。
...
- 当前对 98 篇论文逐篇核查，只收录论文、作者、实验室或机构可确认的官方资源；第三方复现、聚合站和同名仓库不作为开源代码链接。
- 文献卡片、表格、论文详情与专注阅读统一展示“项目主页 / 开源代码”；无可靠官方资源的论文不显示空占位。
- 资源 registry 经过公开 URL / paper ID / kind 校验，并新增浏览器回归验证 OpenVLA 双资源链路及无资源论文的空白行为。
...
- 支持 `$...$`、`$$...$$`、`\\(...\\)`、`\\[...\\]`，以及上下标、分式、根号、求和/积分、希腊字母、常见运算符、accent 与常用矩阵环境。
- 论文详情摘要、深度阅读笔记、专注阅读、对比展开内容和笔记表格统一使用同一数学渲染路径；普通文本继续严格 HTML 转义。
- 解析失败时安全回退原始 TeX，不丢内容、不执行 HTML；移动端块级公式可横向滚动，不撑破页面。
- 新增 Node 数学渲染回归与真实 Chromium MathML 布局检查，覆盖公式尺寸、块级滚动、页面横向溢出和 XSS 转义。
...
- 搜索只使用已加载的轻量 Setting 结构化索引，不全文检索 protocol 正文，也不预加载结果分片；Training Data / Method / Source 行级筛选保持不变。
